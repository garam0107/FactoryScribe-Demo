...
HEADER_ALIASES = {
    ...
    "previous_year_usage_quantity": ["전년도 사용 수량", "전년도 사용수량", "전년도사용수량", "last year usage quantity", "previous year usage quantity"],
    "current_remaining_quantity": ["현재 잔여 수량", "현재 잔여수량", "현재잔여수량", "current remaining quantity", "remaining quantity"],
    "current_year_expected_quantity": ["이번년도 예상 수량", "이번년도 예상수량", "이번년도예상수량", "current year expected quantity", "expected current year quantity"],
            ...
            for row_idx, row in enumerate(rows[header_idx + 1:], start=header_idx + 2):
                ...
                imported_items.append(
                    InventoryItem(
                        ...
                        previous_year_usage_quantity=_to_float(row[header_map["previous_year_usage_quantity"]]) if "previous_year_usage_quantity" in header_map else None,
                        current_remaining_quantity=_to_float(row[header_map["current_remaining_quantity"]]) if "current_remaining_quantity" in header_map else None,
                        current_year_expected_quantity=_to_float(row[header_map["current_year_expected_quantity"]]) if "current_year_expected_quantity" in header_map else None,
        ...
        results.append(
            {
                ...
                "previous_year_usage_quantity": item.previous_year_usage_quantity,
                "current_remaining_quantity": item.current_remaining_quantity,
                "current_year_expected_quantity": item.current_year_expected_quantity,
